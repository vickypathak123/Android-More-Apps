...
import com.example.app.appcenter.newAPI.APIBuilder.getAuthorizationToken
 ...
 */
class GetJsonResponseCoroutineTask<T : Any>(
    ...
    fun execute(
        ...
        apiParams: HashMap<String, String>? = null,
    ...
    private suspend fun doInBackground(
        ...
        apiParams: HashMap<String, String>? = null,
        vararg params: String
    ): Boolean = withContext(Dispatchers.IO + job) {
    ...
    private fun onPostExecute(result: Boolean) {
        if (result) {
            fListener.onSuccess(
                fResponse = getResponseModel(
                    fResponse = response,
                    fModelClass = fModelClass
                )
            )
        } else {
     ...
     */
    private fun makeGetServiceCall(
        reqUrl: String,
     ...
     * @return your json response as a string
     */
    private fun makePostServiceCall(
        reqUrl: String,
        authorization: String? = null,
        apiParams: HashMap<String, String>? = null
    ): String? {
